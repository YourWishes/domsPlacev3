 ...
 */
class DatabaseDroppedChange extends DatabaseChange {
    public function __construct(&$db) {
        parent::__construct($db, VirtualChangeType::$DATABASE_DROPPED);
    }

    public function commit($managed_connection) {
        if(!($managed_connection instanceof ManagedConnection)) throw new Exception();
        
        $query = "DROP DATABASE `" . $this->getDatabase()->getName() . "`;";

...
class UserAddGroupToUser extends \AjaxRequestHandler {
    ...
    public static function getInstance() {
        return UserAddGroupToUser::$INSTANCE;
    }

    public function __construct() {
        parent::__construct('addGroupToUser');

        //As with most AJAX Request listeners, this self registers
        $this->register();
        UserAddGroupToUser::$INSTANCE = $this;
    }
    ...
    public function onRequest(&$request) {
        ...
        if (UsersAddon::getInstance()->isPermissionsAvailable()) {
            if (!Permissions::ADD_GROUP()->canCurrentUser())
        ...
        if ($groups->contains($group))
            $request->send("User already apart of group.");
